...
| `GET` | `/api/reports/writable?modelId=` | ✅ | — | Les rapports de ce modèle que l'appelant peut modifier (`canWriteReport`), workspaces compris : `{ reports: [{ id, title, workspace_id, workspace_name, updated_at, pages: [{id, name}] }] }`. Sert le sélecteur « Add to report » du panneau Ask. |
| `POST` | `/api/reports/:id/widgets` | ✅ | écriture sur le rapport + `canBuildOnModel` | Ajoute des visuels sans passer par l'éditeur. Corps `{ widgets: [{ type, config: {title, subType, sortOrder, dataLimit, visualId}, dataBinding: {…, widgetFilters, timePeriod} }], pageId? }`. Un `customVisual` doit être dans la bibliothèque du workspace DU rapport ; son `bundleUrl` et son manifeste sont reconstruits depuis la bibliothèque, jamais repris du corps. Chaque visuel est **reconstruit** par le validateur de l'assistant contre le modèle effectif DU RAPPORT (400 et rien d'écrit au moindre champ inconnu), puis **placé par le serveur** : sous le contenu de la page, sur une nouvelle page s'il n'y a plus de place ou si c'est un jeu de plusieurs visuels. Une version est conservée. Ne touche ni aux autres pages ni aux autres clés de `settings`. Réponse `{ reportId, pageId, widgetIds, newPage }`. |
...
| Méthode | Path | Auth | Permission | Description |
|---|---|:--:|---|---|
| `GET` | `/api/ai/status` | ✅ | soi-même | `{ enabled, dataSharing, source, reason, personal }` — l'assistant de CET utilisateur (`utils/ai/access.js`). `source` = `instance` ou `personal` ; `reason` quand il n'y en a pas : `off` (interrupteur admin coupé : personne), `denied` (compte refusé par un admin : aucun assistant, même avec sa propre clé), `setup` (l'instance n'a pas de fournisseur : l'utilisateur peut brancher le sien). `personal` = son fournisseur, sans la clé. |
| `PUT` | `/api/ai/personal` | ✅ | soi-même | Fournisseur IA personnel `{ provider, baseUrl, model, apiKey }`, seulement quand l'instance n'en a pas (403 sinon, et pour un compte refusé). Clé chiffrée sur la ligne de l'utilisateur, jamais renvoyée, utilisée pour ses seules requêtes. Le niveau de partage des données reste celui de l'instance. |
| `POST` | `/api/ai/personal/test` | ✅ | soi-même | Teste le fournisseur personnel ENREGISTRÉ, appel d'outil compris : `{ ok, toolCalling, error? }`. |
| `DELETE` | `/api/ai/personal` | ✅ | soi-même | Oublie le fournisseur personnel. |
| `PUT` | `/api/admin/users/:id/ai-access` | ✅ | admin | `{ denied: boolean }` — retire ou rend l'assistant à un compte. `GET /api/admin/users` porte `aiDenied`. `PUT /api/admin/settings/ai` accepte `removeProvider: true` (retour à « chacun apporte le sien »). |
| `POST` | `/api/ai/models/:modelId/chat` | ✅ | `canBuildOnModel` | L'assistant du panneau « Assistant » (à droite des étapes Reports / Data Models / Data Sources) : une conversation sur un MODÈLE, sans rapport ni page. Corps `{ messages, workspaceId? }`. `workspaceId` (403 si l'appelant n'en est pas membre) : sa bibliothèque de visuels personnalisés est proposée au modèle, et un admin de ce workspace se voit offrir `propose_custom_visual`. Ouvert à qui peut construire sur le modèle — pas à qui peut seulement le lire (en OSS, voir un rapport partagé rend son modèle lisible). Seul outil de proposition : `propose_widgets` **sans** `layout` ; tailles et positions sont calculées par le serveur (`arrangeLayout.layoutNew`). Lecture de données : la même, `cacheOnly`, et sans `reportId`. Même budget de 20 req/min que le chat de l'éditeur. Les réponses de chat (ici et dans l'éditeur) portent `reads` — les lectures du cache du tour, 50 lignes max — que le client rejoue dans les tours suivants de la session. 403 compte refusé, 409 assistant coupé. |
| `POST` | `/api/ai/models/:modelId/model-chat` | ✅ | `canWriteModel` | L'assistant de l'éditeur de modèle. Corps `{ messages, draft: { tables: {table: [{column, type}]}, joins, roles, dimensions, measures } }` — le brouillon NON enregistré de l'éditeur, re-projeté côté serveur (noms et types seulement). Proposition `{ kind: 'model', joins, removeJoins, tableRoles, fields, measures, positions }`, contrôlée contre le brouillon (colonnes existantes, pas de boucle de jointures, agrégations sum/avg/count/min/max, aucun SQL) ; appliquée par l'éditeur, enregistrée par l'auteur. |
| `POST` | `/api/ai/feedback` | ✅ | `canBuildOnModel` | 👍/👎 sur une réponse : `{ modelId, answerId, rating: 'up'|'down', question, visuals, surface }`. Une note par utilisateur et par réponse (noter à nouveau la remplace). Ne sont gardés que la question (≤ 500 car.) et la FORME des visuels (`type, subType, title, fields` filtrés par le modèle) — jamais le texte de la réponse ni une ligne de données. |
| `GET` | `/api/admin/ai/feedback?days=30` | 👑 | admin | `{ days, totals: {up, down}, byModel, recent[50] }`. |
| `POST` | `/api/ai/reports/:reportId/chat` | ✅ | écriture sur le rapport | Un tour d'assistant. Corps `{ messages: [{role, text}] (≤ 20), pageContext, mode }`. `mode` = `visuals` (défaut : ajouter des visuels) ou `design` (restyler/déplacer l'existant) ; il fixe les outils offerts au modèle, et un outil de l'autre mode est refusé. Réponse `{ reply, proposals }`. En `design`, le modèle ne fournit ni pixels ni couleurs : il donne des rangées (`arrangement`), une règle d'harmonie (`colorScheme` : categorical, monochromatic, analogous, complementary, split-complementary, triadic, tetradic) et éventuellement une couleur de base (`baseColor`), que le serveur convertit en opérations `move` / `update_config` par des règles fixes (`utils/ai/arrangeLayout.js`, `utils/ai/colorScheme.js`) ; la proposition peut porter des `advice` (texte, rien à appliquer). L'assistant ne lit les données que via `/query` en `cacheOnly`, sous l'identité de l'appelant ; il n'écrit rien : une proposition ne s'applique que côté client, sur action de l'utilisateur. 20 req/min par utilisateur. 409 si désactivé, 502 si le fournisseur échoue. |
...
| `GET` | `/api/workspaces/:wsId/visuals/:visualId/bundle.js` | ✅ | membre du workspace (ou admin) | Sert le bundle JS (`nosniff`, CSP `sandbox` — l'iframe est le consommateur prévu, y naviguer ne doit rien exécuter). En-tête `X-OpenReport-Visual-Origin: upload\|ai` : le client exécute un visuel `ai` avec une CSP sans réseau. |
...
| `POST` | `/api/workspaces/:wsId/visuals` | ✅ | admin du workspace | Installe un paquet `.zip` (manifest + visual.js + icône). `origin = 'upload'`, y compris par-dessus un visuel généré. |
| `POST` | `/api/workspaces/:wsId/visuals/generated` | ✅ | admin du workspace | Ajoute un visuel écrit par l'assistant. Corps JSON `{ manifest, visualJs }`. Id forcé dans l'espace `ai-…`, `origin = 'ai'`, lint réseau rejoué (400), 409 si l'id appartient à un visuel uploadé. |
